...
pub fn get_selection(el: ElementRef, url: &Url) -> HashMap<String, Value> {
    ...
    selection.insert(String::from("tag_name"), json!(tag));
    ...
    if let Some(Value::String(href)) = selection.get("href") {
        if href.starts_with("http") {
            ...
            selection.insert(String::from("href_type"), json!("absolute"));
            selection.insert(String::from("full_href"), json!(href));
        } else {
            let href = href.to_string();
            let url = url.join(&href).expect("full url should be parsable");
            if href.is_empty() {
                selection.insert(String::from("href_type"), json!("empty"));
            } else {
                selection.insert(String::from("href_type"), json!("relative"));
        ...
        if let Some(ext) = ext {
            ...
            if (tag == *"img") || (is_image_format) {
                if let Some(v) = image_formats.into_iter().find(|i| (*i) == ext) {
                    selection.insert(String::from("image_type"), json!(v));
                }
                selection.insert(String::from("source_type"), json!("image"));
            }
        }
    }
